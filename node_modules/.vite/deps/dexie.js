//#region node_modules/dexie/dist/modern/dexie.mjs
...
var NEEDS_THROW_FOR_STACK = !(/* @__PURE__ */ new Error("")).stack;
...
props(DexiePromise.prototype, {
	...
	stack: { get: function() {
		...
		try {
			...
			var stack = getStack(this, [], MAX_LONG_STACKS).join("\nFrom previous: ");
...
function executePromiseTask(promise, fn) {
	try {
		fn((value) => {
			...
			if (value && typeof value.then === "function") executePromiseTask(promise, (resolve, reject) => {
...
function handleRejection(promise, reason) {
	...
	debug && reason !== null && typeof reason === "object" && !reason._promise && tryCatch(() => {
		...
		setProp(reason, "stack", { get: () => stack_being_generated ? origProp && (origProp.get ? origProp.get.apply(reason) : origProp.value) : promise.stack });
	});
...
var Table = class {
	...
	where(indexOrCrit) {
		...
		const compoundIndex = this.schema.indexes.concat(this.schema.primKey).filter((ix) => {
			if (ix.compound && keyPaths.every((keyPath) => ix.keyPath.indexOf(keyPath) >= 0)) {
				for (let i = 0; i < keyPaths.length; ++i) if (keyPaths.indexOf(ix.keyPath[i]) === -1) return false;
				return true;
			}
			return false;
	...
	update(keyOrObject, modifications) {
		if (typeof keyOrObject === "object" && !isArray(keyOrObject)) {
			...
			try {
				if (typeof modifications !== "function") keys(modifications).forEach((keyPath) => {
	...
	bulkAdd(objects, keysOrOptions, options) {
		const keys = Array.isArray(keysOrOptions) ? keysOrOptions : void 0;
...
function iterate(cursorPromise, filter, fn, valueMapper) {
	var wrappedFn = wrap(valueMapper ? (x, c, a) => fn(valueMapper(x), c, a) : fn);
	return cursorPromise.then((cursor) => {
		if (cursor) return cursor.start(() => {
...
var Collection = class {
	...
	offset(offset) {
		...
		ctx.offset += offset;
		if (isPlainKeyRange(ctx)) addReplayFilter(ctx, () => {
	...
	primaryKeys(cb) {
		var ctx = this._ctx;
		if (ctx.dir === "next" && isPlainKeyRange(ctx, true) && ctx.limit > 0) return this._read((trans) => {
	...
	modify(changes) {
		...
		return this._write((trans) => {
			...
			const applyMutateResult = (expectedCount, res) => {
				...
				for (let pos of keys(failures)) totalFailures.push(failures[pos]);
			};
				...
				const nextChunk = (offset) => {
					...
					return coreTable.getMany({
						...
						for (let i = 0; i < count; ++i) {
							...
							if (modifyer.call(ctx, ctx.value, ctx) !== false) if (ctx.value == null) deleteKeys.push(keys[offset + i]);
						...
						return Promise.resolve(addValues.length > 0 && coreTable.mutate({
							...
							for (let pos in res.failures) deleteKeys.splice(parseInt(pos), 1);
	...
	delete() {
		...
		if (isPlainKeyRange(ctx) && (ctx.isPrimKey && !hangsOnDeleteLargeKeyRange || range.type === 3)) return this._write((trans) => {
...
function addIgnoreCaseAlgorithm(whereClause, match, needles, suffix) {
	...
	if (!needles.every((s) => typeof s === "string")) return fail(whereClause, STRING_EXPECTED);
...
var WhereClause = class {
	...
	inAnyRange(ranges, options) {
		...
		if (!ranges.every((range) => range[0] !== void 0 && range[1] !== void 0 && ascending(range[0], range[1]) <= 0)) return fail(this, "First argument to inAnyRange() must be an Array of two-value Arrays [lower,upper] where upper must not be lower than lower", exceptions.InvalidArgument);
		...
		c._addAlgorithm((cursor, advance, resolve) => {
			...
			else if (this._cmp(key, set[rangePos][1]) === 0 || this._cmp(key, set[rangePos][0]) === 0) return false;
			else {
				advance(() => {
	...
	startsWithAnyOf() {
		const set = getArrayOf.apply(NO_CHAR_ARRAY, arguments);
		if (!set.every((s) => typeof s === "string")) return fail(this, "startsWithAnyOf() only works with strings");
		if (set.length === 0) return emptyCollection(this);
...
var Transaction = class {
	...
	_promise(mode, fn, bWriteLock) {
		...
		if (this._locked()) return new DexiePromise((resolve, reject) => {
			this._blockedFuncs.push([() => {
				this._promise(mode, fn, bWriteLock).then(resolve, reject);
			}, PSD]);
		});
		else if (bWriteLock) return newScope(() => {
	...
	waitFor(promiseLike) {
		...
		if (root._waitingFor) root._waitingFor = root._waitingFor.then(() => promise);
		else {
...
function getKeyExtractor(keyPath) {
	if (keyPath == null) return () => void 0;
	else if (typeof keyPath === "string") return getSinglePathKeyExtractor(keyPath);
	else return (obj) => getByKeyPath(obj, keyPath);
}
function getSinglePathKeyExtractor(keyPath) {
	if (keyPath.split(".").length === 1) return (obj) => obj[keyPath];
	else return (obj) => getByKeyPath(obj, keyPath);
}
		...
		function mutate({ trans, type, keys, values, range }) {
			return new Promise((resolve, reject) => {
				...
				if (type === "deleteRange") {
					...
					if (isAddOrPut) for (let i = 0; i < length; ++i) {
		...
		return {
			...
			getMany({ trans, keys }) {
				return new Promise((resolve, reject) => {
					...
					for (let i = 0; i < length; ++i) if (keys[i] != null) {
			...
			get({ trans, key }) {
				return new Promise((resolve, reject) => {
					resolve = wrap(resolve);
					const req = trans.objectStore(tableName).get(key);
...
function setApiOnPlace({ _novip: db }, objs, tableNames, dbschema) {
	tableNames.forEach((tableName) => {
		...
		objs.forEach((obj) => {
			...
			if (!propDesc || "value" in propDesc && propDesc.value === void 0) if (obj === db.Transaction.prototype || obj instanceof db.Transaction) setProp(obj, tableName, {
				get() {
...
function removeTablesApi({ _novip: db }, objs) {
	objs.forEach((obj) => {
		for (let key in obj) if (obj[key] instanceof db.Table) delete obj[key];
	});
}
...
function updateTablesAndIndexes({ _novip: db }, oldVersion, trans, idbUpgradeTrans) {
	...
	versions.filter((v) => v._cfg.version >= oldVersion).forEach((version) => {
		queue.push(() => {
			...
			if (contentUpgrade && version._cfg.version > oldVersion) {
				...
				if (contentUpgradeIsAsync) incrementExpectedAwaits();
				let returnValue;
...
function buildGlobalSchema(db, idbdb, tmpTrans) {
	const globalSchema = {};
	slice(idbdb.objectStoreNames, 0).forEach((storeName) => {
...
function verifyInstalledSchema(db, tmpTrans) {
	const diff = getSchemaDiff(buildGlobalSchema(db, db.idbdb, tmpTrans), db._dbSchema);
...
function dexieOpen(db) {
	...
	const tryOpenDB = () => new DexiePromise((resolve, reject) => {
		...
		if (err && err.name === "UnknownError" && state.PR1398_maxLoop > 0) {
			...
			return tryOpenDB();
		} else return DexiePromise.reject(err);
	});
...
function createVirtualIndexMiddleware(down) {
	return {
		...
		table(tableName) {
			...
			for (const index of schema.indexes) addVirtualIndexes(index.keyPath, 0, index);
				...
				openCursor(req) {
					...
					function createVirtualCursor(cursor) {
						...
						return Object.create(cursor, {
							...
							continuePrimaryKey: { value(key, primaryKey) {
								...
								if (req.type === "delete") deleting.fire.call(ctx, key, existingValue, dxTrans);
									...
									if (additionalChanges) {
										...
										Object.keys(additionalChanges).forEach((keyPath) => {
											if (hasOwn(requestedValue, keyPath)) requestedValue[keyPath] = additionalChanges[keyPath];
											else setByKeyPath(requestedValue, keyPath, additionalChanges[keyPath]);
										});
									}
								}
								return ctx;
					...
					function deleteNextChunk(trans, range, limit) {
						return downTable.query({
							...
							return addPutOrDelete({
								...
								else return deleteNextChunk(trans, {
									...range,
				...
				keys(readSubscribers).forEach((method) => {
					tableClone[method] = function(req) {
						...
						if (subscr) {
							...
							if (!queriedIndex.isPrimaryKey) if (method === "count") delsRangeSet.add(FULL_RANGE);
							else {
								...
								return table[method].apply(this, arguments).then((res) => {
									if (method === "query") {
										if (outbound && req.values) return keysPromise.then(({ result: resultingKeys }) => {
											pkRangeSet.addKeys(resultingKeys);
											return res;
										});
										const pKeys = req.values ? res.result.map(extractKey) : res.result;
										if (req.values) pkRangeSet.addKeys(pKeys);
										else delsRangeSet.addKeys(pKeys);
...
var Dexie$1 = class Dexie$1 {
	...
	_whenReady(fn) {
		return this.idbdb && (this._state.openComplete || PSD.letThrough || this._vip) ? fn() : new DexiePromise((resolve, reject) => {
			if (this._state.openComplete) return reject(new exceptions.DatabaseClosed(this._state.dbOpenError));
	...
	unuse({ stack, name, create }) {
		if (stack && this._middlewares[stack]) this._middlewares[stack] = this._middlewares[stack].filter((mw) => create ? mw.create !== create : name ? mw.name !== name : false);
		return this;
	}
	open() {
	...
	_transaction(mode, tables, scopeFunc) {
		...
		try {
			...
			if (parentTransaction) {
				...
				if (parentTransaction) storeNames.forEach((storeName) => {
...
if (typeof dispatchEvent !== "undefined" && typeof addEventListener !== "undefined") {
	globalEvents(DEXIE_STORAGE_MUTATED_EVENT_NAME, (updatedParts) => {
		if (!propagatingLocally) {
			...
			} else event = new CustomEvent(STORAGE_MUTATED_DOM_EVENT_NAME, { detail: updatedParts });
...
if (typeof BroadcastChannel !== "undefined") {
	...
	globalEvents(DEXIE_STORAGE_MUTATED_EVENT_NAME, (changedParts) => {
		try {
			if (!propagatingLocally) {
				...
				if (typeof self["clients"] === "object") [...self["clients"].matchAll({ includeUncontrolled: true })].forEach((client) => client.postMessage({
	...
	if (typeof addEventListener !== "undefined") addEventListener("storage", (ev) => {
...
//#endregion
export { Dexie$1 as Dexie, Dexie$1 as default, RangeSet, liveQuery, mergeRanges, rangesOverlap };
